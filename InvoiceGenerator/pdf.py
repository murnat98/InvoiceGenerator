...
class SimpleInvoice(BaseInvoice):
    ...
    def gen(self, filename, generate_qr_code=False, draw_payment=True, draw_creator=True):
        """
        Generate the invoice into file
        ...
        :param draw_payment: should be payment information included in the PDF?
        :type draw_payment: boolean
        :param draw_creator: should be creator information included in the PDF?
        :type draw_payment: boolean
        """
        ...
        self._draw_payment = draw_payment
        self._draw_creator = draw_creator
    ...
    def _drawAddress(self, top, left, width, height, header_string, address):
        ...
        if address.logo_filename:
            ...
            self.pdf.drawImage(self.invoice.provider.logo_filename, (left + 84) * mm - width, (top - 4) * mm, width,
                               height, mask="auto")
    ...
    def _drawItems(self, TOP, LEFT):  # noqa
        ...
        for item in self.invoice.items:
            ...
            if will_wrap and TOP - i - i_add < 8 * mm:
                will_wrap = False
                self.pdf.rect(LEFT * mm, (TOP - i) * mm, (LEFT + 156) * mm, (i + 2) * mm, stroke=True,
                              fill=False)  # 140,142
                self.pdf.showPage()
            ...
            if items_are_with_tax:
                ...
                self.pdf.drawRightString((LEFT + 173) * mm, (TOP - i) * mm,
        ...
        if self.invoice.rounding_result:
            ...
            self.pdf.drawString((LEFT + 68) * mm, (TOP - i) * mm,
                                currency(self.invoice.difference_in_rounding, self.invoice.currency,
        ...
        if not items_are_with_tax:
            ...
            self.pdf.drawString((LEFT + 100) * mm, (TOP - i - 7) * mm, '%s: %s' % (
            _(u'Total'), currency(self.invoice.price, self.invoice.currency, self.invoice.currency_locale)))
        else:
            self.pdf.setFont('DejaVu-Bold', 6)
            self.pdf.drawString((LEFT + 1) * mm, (TOP - i - 2) * mm, _(u'Breakdown VAT'))
            vat_list, tax_list, total_list, total_tax_list = [_(u'VAT rate')], [_(u'Tax')], [_(u'Without VAT')], [
                _(u'With VAT')]
            ...
            self.pdf.drawString(
                ...
                u'%s: %s' % (_(u'Total with tax'),
                             currency(self.invoice.price_tax, self.invoice.currency, self.invoice.currency_locale)),
            )

        if items_are_with_tax:
            self.pdf.rect(LEFT * mm, (TOP - i - 17) * mm, (LEFT + 156) * mm, (i + 19) * mm, stroke=True,
                          fill=False)  # 140,142
        else:
            self.pdf.rect(LEFT * mm, (TOP - i - 11) * mm, (LEFT + 156) * mm, (i + 13) * mm, stroke=True,
                          fill=False)  # 140,142

        if self._draw_creator:
    ...
    def _drawCreator(self, TOP, LEFT):
        ...
        self.pdf.drawString((LEFT + 10) * mm, (TOP - 5) * mm - height,
    ...
    def _drawDates(self, TOP, LEFT):
        ...
        if self.invoice.date and self.invoice.use_tax:
            items.append((LEFT * mm, '%s` %s' % (
            _(u'Date of exposure taxable invoice'), format_date(self.invoice.date, locale=lang))))
        ...
        if self.invoice.taxable_date:
            items.append(
                (LEFT * mm, '%s` %s' % (_(u'Taxable date'), format_date(self.invoice.taxable_date, locale=lang))))
